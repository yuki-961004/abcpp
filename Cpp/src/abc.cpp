...
std::string matrix_shape(const Matrix& matrix) {
...
Matrix flatten_sumstat_matrices(
    const Matrix& target,
    const Matrix& param,
    const std::vector<Matrix>& sumstats
) {
    const std::size_t target_size = target.data().size();
    if (target_size == 0) {
        throw std::invalid_argument(
            "abc() expected target matrix to contain at least one value."
        );
    }
    if (sumstats.size() != param.rows()) {
        std::ostringstream message;
        message << "abc() expected sumstats list length "
                << sumstats.size()
                << " to equal param.rows() " << param.rows()
                << " for param shape " << matrix_shape(param) << ".";
        throw std::invalid_argument(message.str());
    }

    Matrix out(param.rows(), target_size, 0.0);
    for (std::size_t row = 0; row < sumstats.size(); ++row) {
        const Matrix& current = sumstats[row];
        if (current.data().size() != target_size) {
            std::ostringstream message;
            message << "abc() expected sumstats["
                    << row
                    << "] flattened dimension "
                    << current.data().size()
                    << " to equal target flattened dimension "
                    << target_size
                    << "; target shape is " << matrix_shape(target)
                    << " and sumstats[" << row << "] shape is "
                    << matrix_shape(current) << ".";
            throw std::invalid_argument(message.str());
        }
        for (std::size_t col = 0; col < target_size; ++col) {
            out(row, col) = current.data()[col];
        }
    }
    return out;
}
...
Matrix reshape_sumstat_for_param(
    ...
    if (simulation_count == 0 ||
        ...
        std::ostringstream message;
        message << "abc() cannot reshape sumstat for param rows: "
                << "param.rows() is " << simulation_count
                << ", sumstat shape is " << matrix_shape(sumstat)
                << ", and sumstat entries are not divisible by param rows.";
        throw std::invalid_argument(message.str());
    }
...
AbcResult abc(
    ...
    if (raw_param.rows() != raw_sumstat.rows()) {
        std::ostringstream message;
        message << "abc() expected param.rows() == sumstat.rows(), got "
                << "param shape " << matrix_shape(raw_param)
                << " and sumstat shape " << matrix_shape(raw_sumstat) << ".";
        throw std::invalid_argument(message.str());
    }
    if (raw_param.cols() == 0) {
        std::ostringstream message;
        message << "abc() expected param.cols() > 0, got param shape "
                << matrix_shape(raw_param) << ".";
        throw std::invalid_argument(message.str());
    }
    if (raw_sumstat.cols() == 0) {
        std::ostringstream message;
        message << "abc() expected sumstat.cols() > 0, got sumstat shape "
                << matrix_shape(raw_sumstat) << ".";
        throw std::invalid_argument(message.str());
    }
    if (raw_target.size() != raw_sumstat.cols()) {
        std::ostringstream message;
        message << "abc() expected target summary dimension "
                << raw_target.size()
                << " to equal sumstat.cols() " << raw_sumstat.cols()
                << " for sumstat shape " << matrix_shape(raw_sumstat) << ".";
        throw std::invalid_argument(message.str());
    }
    if (!raw_options.subset.empty() &&
        raw_options.subset.size() != raw_sumstat.rows()) {
        std::ostringstream message;
        message << "abc() expected subset length "
                << raw_options.subset.size()
                << " to equal sumstat.rows() " << raw_sumstat.rows()
                << ".";
        throw std::invalid_argument(message.str());
    }
