...
abcpp::Matrix make_param_matrix(std::size_t rows, std::size_t cols) {
    ...
    for (std::size_t row = 0; row < rows; ++row) {
        ...
        for (std::size_t col = 0; col < cols; ++col) {
            // 涓枃璇存槑: 姣忎竴鍒椾娇鐢ㄤ笉鍚屾枩鐜? 闃叉鍥炲綊娴嬭瘯閫€鍖栨垚甯告暟鍒?
...
abcpp::Matrix make_sumstat_matrix(std::size_t rows, std::size_t cols) {
    ...
    for (std::size_t row = 0; row < rows; ++row) {
        ...
        for (std::size_t col = 0; col < cols; ++col) {
            // 涓枃璇存槑: 姝ｅ鸡鍜岀嚎鎬ч」娣峰悎, 璁?PCA, PLS, 鍥炲綊閮芥湁鐪熷疄淇″彿.
...
void test_reductions_keep_requested_dimension() {
    ...
    require_true(clipped.info.ncomp == 5, "pca clips component count");
...
void test_reduction_none_and_pls_contracts() {
    ...
    require_true(none_result.reduction.ncomp == 4,
    ...
    require_true(none_reduced.info.ncomp == sumstat.cols(),
    ...
    require_true(pls_reduced.info.ncomp == 2,
    ...
    require_true(pls_result.reduction.ncomp == 2,
